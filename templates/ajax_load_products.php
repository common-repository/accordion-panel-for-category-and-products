<?php if ( ! defined( 'ABSPATH' ) ) exit; 
    ...
    echo "<input type='hidden' value='".$category_id."' class='ik-drp-product-category' />"; 
	$_total_products = $this->getTotalProducts( $category_id, 1, $is_default_category_with_hidden );
	if( $_total_products <= 0 ) {
		?><div class="ik-product-no-items"><?php echo __( 'No products found.', 'categoryaccordionpanel' ); ?></div><?php
		die();
	} 
	$product_list = $this->getProductList( $category_id, $_limit_end );	 
	...
	if( $_total_products > sanitize_text_field( $params["number_of_product_display"] ) ) { ?>
			<div class="clr"></div>
			<div class='ik-product-load-more'  align="center" onclick='ACP_loadMoreProducts( <?php echo esc_js( $category_id ); ?>, "<?php echo esc_js( $_limit_start+$_limit_end ); ?>", "<?php echo esc_js( $params["vcode"]."-".$category_id ); ?>", "<?php echo esc_js( $_total_products ); ?>", request_obj_<?php echo esc_js( $params["vcode"] ); ?> )'>
